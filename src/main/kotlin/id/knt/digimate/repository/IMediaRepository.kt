...
@Repository(value = "mediaRepository") interface IMediaRepository : JpaRepository<Media, String> {
	...
	@Query(value = "select m.id, m.title, m.description, m.type, m.isPublished, m.language, m.fileUrl, m.user.id from" +
			...
			" " +
			"join " +
			"fetch m.user where m.language= ?1 and m.isPublished = true and m.type='image'")
	...
	@Query(value = "select m.id, m.title, m.description, m.type, m.youtubeUrl, m.isPublished, m.language, m.fileUrl, m.user.id " +
			"from Media m join fetch m.user where m.language= ?1 and m.isPublished = true and m.type='video'")
